...
data_601318 <- sqlQuery("select a.`dt`, a.`open`, a.`high`, a.`low`, a.`close`, a.`volume`, a.`amount` from (SELECT `dt`, `open`, `high`, `low`, `close`, `volume`, `amount` FROM `ying_calc`.`s_xts_adj` where ids = '601318' order by `dt` desc limit 98) as a order by a.`dt`;")
...
# beginning of 600030
data_600030 <- sqlQuery("select a.`dt`, a.`open`, a.`high`, a.`low`, a.`close`, a.`volume`, a.`amount` from (SELECT `dt`, `open`, `high`, `low`, `close`, `volume`, `amount` FROM `ying_calc`.`s_xts_adj` where ids = '600030' order by `dt` desc limit 98) as a order by a.`dt`;")
