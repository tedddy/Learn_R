...
#ibm <- merge(Cl(IBM), MACD(Cl(IBM), 12, 26, 9, "EMA", FALSE))
...
res=dbSendQuery(dbh, "select a.`dt`, a.`open`, a.`high`, a.`low`, a.`close`, a.`volume`, a.`amount` from (SELECT `dt`, `open`, `high`, `low`, `close`, `volume`, `amount` FROM `ying_calc`.`s_xts_adj` where ids = '601318' order by `dt` desc limit 98) as a order by a.`dt`;")
...
hs601318<-merge(Cl(data_xts_601318), MACD(Cl(data_xts_601318), 12, 26, 9, "EMA", FALSE))

hs601318$macdOsc <- hs601318$macd - hs601318$signal           

View(tail(hs601318, n=25L))

# beginning of 600030
res=dbSendQuery(dbh, "select a.`dt`, a.`open`, a.`high`, a.`low`, a.`close`, a.`volume`, a.`amount` from (SELECT `dt`, `open`, `high`, `low`, `close`, `volume`, `amount` FROM `ying_calc`.`s_xts_adj` where ids = '600030' order by `dt` desc limit 98) as a order by a.`dt`;")
data_600030 =fetch(res)
rownames(data_600030) = data_600030[,1]
data_xts_temp <- data_600030[,-1]
data_xts_600030 <- as.xts(data_xts_temp)
hs600030<-merge(Cl(data_xts_600030), MACD(Cl(data_xts_600030), 12, 26, 9, "EMA", FALSE))
hs600030$macdOsc <- hs600030$macd - hs600030$signal        
View(tail(hs600030, n=25L))
# end of 600030
...
hs600036<-merge(Cl(data_xts_600036), MACD(Cl(data_xts_600036), 12, 26, 9, "EMA", FALSE))
hs600036$macdOsc <- hs600036$macd - hs600036$signal        
View(tail(hs600036, n=25L))
# end of 600036
...
hs600016<-merge(Cl(data_xts_600016), MACD(Cl(data_xts_600016), 12, 26, 9, "EMA", FALSE))
hs600016$macdOsc <- hs600016$macd - hs600016$signal        
View(tail(hs600016, n=25L))
# end of 600016

# beginning of 600000
res=dbSendQuery(dbh, "select a.`dt`, a.`open`, a.`high`, a.`low`, a.`close`, a.`volume`, a.`amount` from (SELECT `dt`, `open`, `high`, `low`, `close`, `volume`, `amount` FROM `ying_calc`.`s_xts_adj` where ids = '600000' order by `dt` desc limit 98) as a order by a.`dt`;")
data_600000 =fetch(res)
rownames(data_600000) = data_600000[,1]
data_xts_temp <- data_600000[,-1]
data_xts_600000 <- as.xts(data_xts_temp)
hs600000<-merge(Cl(data_xts_600000), MACD(Cl(data_xts_600000), 12, 26, 9, "EMA", FALSE))
hs600000$macdOsc <- hs600000$macd - hs600000$signal        
View(tail(hs600000, n=25L))
# end of 600000

# beginning of 601166
res=dbSendQuery(dbh, "select a.`dt`, a.`open`, a.`high`, a.`low`, a.`close`, a.`volume`, a.`amount` from (SELECT `dt`, `open`, `high`, `low`, `close`, `volume`, `amount` FROM `ying_calc`.`s_xts_adj` where ids = '601166' order by `dt` desc limit 98) as a order by a.`dt`;")
data_601166 =fetch(res)
rownames(data_601166) = data_601166[,1]
data_xts_temp <- data_601166[,-1]
data_xts_601166 <- as.xts(data_xts_temp)
hs601166<-merge(Cl(data_xts_601166), MACD(Cl(data_xts_601166), 12, 26, 9, "EMA", FALSE))
hs601166$macdOsc <- hs601166$macd - hs601166$signal        
View(tail(hs601166, n=25L))
# end of 601166
