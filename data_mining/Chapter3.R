...
# e start
quotes <- last(GSPC,'3 months')
View(quotes)
nrow(quotes)
tgt.margin <- 0.025 # target margin
n.days <- 10 # number of days
v <- apply(HLC(quotes),1,mean) # average price
View(v)
v_xts <- as.xts(v)
index(v_xts) <- as.POSIXct(format(time(v_xts)),tz="")
index(quotes) <- as.POSIXct(format(time(quotes)),tz="")
index(v_xts) 
index(quotes)

quotes_v_xts <- cbind(quotes, v_xts)
quotes_v_xts <- merge(quotes, v_xts)
View(quotes_v_xts)

r <- matrix(NA,ncol=n.days,nrow=NROW(quotes)) # matrix with NA
dv <- Delt(v,k=1) # the one period percent change of the series is returned
ndv <- Next(Delt(v,k=1)) # Create a new series with all values advanced forward one period. The value of period 1, becomes the value at period 2, value at 2 becomes the original value at 3, etc. The opposite of Lag. NA is used to fill.
ldv <- Lag(Delt(v,k=1)) # Lag series
...
addAvgPrice(on=1) # the indicator will be plotted on the first graph window
addT.ind()
