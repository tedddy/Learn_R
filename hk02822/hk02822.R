...
res=dbSendQuery(dbh, "SELECT `date`, `open`, `high`, `low`, `close`, `volume`, `amount` FROM `ying`.`hk_s_xts` where code = '02822' and datediff(curdate(), `date`) < 88 order by date;")
...
hk02822<-merge(Cl(data_xts), MACD(Cl(data_xts), 8, 17, 9, "EMA", FALSE))

hk02822$macdOsc <- hk02822$macd - hk02822$signal           
