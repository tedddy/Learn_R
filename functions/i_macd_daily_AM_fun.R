...
fun_i_macd_daily_AM <- function (idi, limit = '66') {
    # setup connection to database
    dbh <- dbConnect(MySQL(), user="gxh", password="locoy", dbname="ying", host="192.168.137.172")
    # make query 
    query <- paste("SELECT dt as date, high as High, low as Low, close as Close, amount as Volume FROM ying.i_AM where `idi` =",idi," and `dt` <> '0000-00-00' ORDER BY `dt` DESC LIMIT ", limit, ";") # Note that we use amount as volume in the xts since volume has no much meaning for an index.
    # mysql result
    ...
    dbSendQuery(dbh,'call ying.i_macd_AM_import();')
    # disconnect database
    dbDisconnect(dbh)
}
...
#     dbh <- dbConnect(MySQL(), user="gxh", password="locoy", dbname="ying", host="192.168.137.172")
#     # make query 
#     query <- paste("SELECT dt as date, high as High, low as Low, close as Close, amount as Volume FROM ying.i_AM where `idi` =",idi," ORDER BY `dt` DESC LIMIT ", limit, ";") # Note that we use amount as volume in the xts since volume has no much meaning for an index.
#     # mysql result
#     result <- dbSendQuery(dbh, query)
#     # fetch data
#     ttrc <- fetch(result, -1)
...
# ttrc <- sqlQuery(paste("SELECT dt as date, high as High, low as Low, close as Close, amount as Volume FROM ying.i_AM where `idi` =",idi," ORDER BY `dt` DESC LIMIT ", limit, ";")) # Note that we use amount as volume in the xts since volume has no much meaning for an index.
...
# 
# dbSendQuery(dbh,'call ying.i_macd_bl_archive_import();')
# 
# macd_data.frame <- tail(macd_data.frame, n=66L)
# 
# dbSendQuery(dbh,'call ying.i_macd_bl_import();')
# 
# dbDisconnect(dbh)
