...
dbh <- dbConnect(MySQL(), user="gxh", password="locoy", dbname="ying_calc", host="192.168.137.172")
 ...
 list_e <-as.data.frame(read_excel("E:\\user_tony\\download\\hk_w_sg\\list_e_2016-03-07.xls", sheet = 1, col_names = TRUE, na = "",skip = 0))
 
 #my.dataframe$new.col <- a.vector
 # list_e$dt <- Sys.Date()
 list_e$dt <- '2016-03-07'
 
 #View(list_e)
 
 dbWriteTable(dbh, 'hk_w_sg_imported', list_e, row.names = FALSE, overwrite = TRUE)
 dbSendQuery(dbh,'call ying_calc.hk_w_sg_import();')
